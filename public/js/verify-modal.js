...
;(() => {
  ...
  // --- Read UI picks from main page (best-effort) ---
  ...
  function readUIPredictions() {
    ...
    try {
      ...
      const pick = sel => (scope.querySelector(sel)?.textContent || "").trim();

      picks.win   = pick("[data-pick='win'], .pick-win b, .emoji-win~b");

      picks.place = pick("[data-pick='place'], .pick-place b, .emoji-place~b");

      picks.show  = pick("[data-pick='show'], .pick-show b, .emoji-show~b");
  ...
  // --- Green-Zone placeholder scoring (will be replaced by calibrated profiles) ---
  ...
  function updateGreenZoneToday(host) {
    ...
    summary.innerHTML = `<b>Suggested Bets (Today):</b> Win-Only ${counts.WinOnly || 0} • ATB ${counts.ATB || 0} • Exacta Box ${counts.ExactaBox || 0} • Trifecta Box ${counts.TrifectaBox || 0}
      ...
      <span title="Current tiers are placeholders until calibrated: Green ≥72, Yellow 58–71, Red <58 • Based on confidence, top-3 mass and gaps." style="opacity:.6;margin-left:8px;cursor:help">ℹ️</span>`;
  ...
  function buildModal() {
    ...
    host.innerHTML = `
      ...
      <div role="dialog" aria-modal="true" class="flv-card" style="width:min(880px,96vw);max-height:90vh;overflow:auto;border-radius:16px;border:1px solid rgba(255,255,255,.12);background:rgba(23,23,28,.98);backdrop-filter:blur(6px);padding:18px;">
        ...
        <!-- Field row: resilient flex with wrap -->

        <div style="display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap" id="flv-field-row">

          <label style="flex:1 1 280px;min-width:260px;">
          ...
          <label style="flex:0 0 180px;min-width:160px;">

            <div style="margin-bottom:6px;opacity:.9">Date (YYYY-MM-DD)</div>

            <input id="flv-date" type="date"

                   style="width:100%;min-width:160px;padding:10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:#1e1f24;color:#fff;appearance:auto!important"/>
    ...
    const rawEl      = qs("#flv-sum-body", host); // reuse for errors/messages
    ...
    if (runBtn) {
      ...
      runBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          // Fire-and-forget backfill to resolve pending logs (ignore failures)
  ...
  function prefill(host, ctx) {
    ...
    // Seed today's snapshot so Green-Zone has at least one row
