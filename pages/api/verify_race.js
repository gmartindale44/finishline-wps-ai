...
function parseHRNRaceOutcome($, raceNo) {
  ...
  try {
    ...
    const getRaceFromNode = (node) => {
      ...
      if (!text) return null;
      // Match patterns like "Race 1", "Race #1", "Race: 1", etc.
    ...
    $("table").each((_, table) => {
      ...
      // Extract header texts - cheerio's .text() already includes nested element text
      // This handles cases like <abbr title="...">Runner (Speed)</abbr>
      const headerTexts = headerCells.map((cell) => {
        // .text() gets all text content including nested elements
        return $(cell).text().toLowerCase().trim();
      });
      ...
      // More flexible matching: allow "Runner (Speed)" or just "Runner" or "Horse"
      ...
      debugInfo.candidateTables.push({
        headerTexts: headerTexts,
        runnerIdx,
        winIdx,
        placeIdx,
      ...
      // 1a) Look at previous siblings of the table (up to 10 elements)
      let prev = $table.prev();
      let prevCount = 0;
      while (prev.length && !inferredRaceNo && prevCount < 10) {
      ...
      if (!inferredRaceNo) {
        ...
        while (parent.length && depth < 6 && !inferredRaceNo) {
          let sib = parent.prev();
          let sibCount = 0;
          while (sib.length && !inferredRaceNo && sibCount < 10) {
      ...
      // 1c) If still not found, check the table's own header row for race number
      if (!inferredRaceNo) {
        const headerText = headerRow.text();
        const raceMatch = headerText.match(/race\s*[:#]?\s*(\d+)/i);
        if (raceMatch) {
          inferredRaceNo = raceMatch[1];
        }
      }
    ...
    if (!runnerTables.length) {
      console.warn("[verify_race][hrn] No WPS tables found", {
        requestedRaceNo,
        debugInfo: {
          totalTables: debugInfo.allTables.length,
          candidateCount: debugInfo.candidateTables.length,
          sampleHeaders: debugInfo.allTables.slice(0, 3).map(t => t.headerTexts),
        },
      });
      return outcome;
    }
    ...
    if (!extracted.win && !extracted.place && !extracted.show) {
      console.warn("[verify_race][hrn] extractOutcomeFromRunnerTable returned empty outcome", {
        requestedRaceNo,
        tableRowCount: $runnerTable.find("tr").length,
  ...
  try {
    ...
    if (redis) {
      ...
      try {
        ...
        debugNotes.push({
          where: "redis_logging",
          error: `redis_unreachable: ${error?.message || String(error)}`,
        });
      }
    }
