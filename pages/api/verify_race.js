...
function extractOutcomeFromRunnerTable($, table, idx) {
  ...
  // Helper: safely extract horse name from runner at index (position-based)
  const safeHorseNameAtIndex = (idx) => {
    if (idx < 0 || idx >= runners.length) return "";
  ...
  // 3) SHOW:
  // First try: Use position-based extraction (safest - rows are ordered by finish position)
  // Find the 3rd distinct horse (after WIN and PLACE) from the runners array
  let showHorse = "";
  
  if (winHorse && placeHorse && runners.length >= 3) {
    // Find the 3rd distinct finisher (skip WIN and PLACE)
    for (let i = 0; i < runners.length; i++) {
      const candidate = safeHorseNameAtIndex(i);
      if (candidate && candidate !== winHorse && candidate !== placeHorse) {
        showHorse = candidate;
        break; // Use the first distinct horse after WIN/PLACE
      }
    }
  }
  
  // Fallback: Try payout-based extraction if position-based didn't work
  if (!showHorse) {
    showHorse =
      runners.find(
        (r) =>
