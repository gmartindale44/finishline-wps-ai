...
function parseHRNRaceOutcome($, raceNo, track = "") {
  ...
  try {
    ...
    // Step 3: Extract horse names from the WPS payout table
    // This table has columns: Runner (Speed) | Win | Place | Show
    // Classification:
    // - Row with Win+Place+Show = WIN horse
    ...
    // Find column indices for Runner, Win, Place, Show
    const runnerColIdx = headerTexts.findIndex((h) => 
      h.includes("runner") && !h.includes("jockey") && !h.includes("trainer")
    );
    const winColIdx = headerTexts.findIndex((h) => h.includes("win"));
    const placeColIdx = headerTexts.findIndex((h) => h.includes("place"));
    const showColIdx = headerTexts.findIndex((h) => h.includes("show"));

    // Bail out if we don't have the required columns
    if (runnerColIdx === -1 || winColIdx === -1 || placeColIdx === -1 || showColIdx === -1) {
      console.warn("[verify_race_full] HRN WPS table missing required columns", {
        runnerColIdx,
        winColIdx,
        placeColIdx,
        showColIdx,
    ...
    // Walk through rows and classify by WPS payouts
    let winHorse = null;
    let placeHorse = null;
    let showHorse = null;

    $resultsTable.find("tr").slice(1).each((_, row) => { // Skip header row
      if (winHorse && placeHorse && showHorse) return false; // Found all, break

      const $row = $(row);
      const cells = $row.find("td, th").toArray();
      if (cells.length === 0) return;

      // Get runner text from runner column
      let runnerText = "";
      if (runnerColIdx >= 0 && runnerColIdx < cells.length) {
        const $cell = $(cells[runnerColIdx]);
        // Prefer link text (horse names are often in links)
        runnerText = $cell.find("a").first().text().trim() || $cell.text().trim();
      }

      if (!runnerText) return;

      // Skip header rows
      const runnerLower = runnerText.toLowerCase();
      if (runnerLower === "runner" || runnerLower === "horse" || runnerLower.includes("speed")) {
        return;
      }

      // Get Win/Place/Show payout text
      const getPayoutText = (idx) => {
        if (idx < 0 || idx >= cells.length) return "";
        return $(cells[idx]).text().trim();
      };

      let winText = getPayoutText(winColIdx);
      let placeText = getPayoutText(placeColIdx);
      let showText = getPayoutText(showColIdx);

      // Normalize payout cells: strip $, commas, asterisks; treat "", "-", "--" as empty
      const normalizePayout = (text) => {
        if (!text) return "";
      ...
      winText = normalizePayout(winText);
      placeText = normalizePayout(placeText);
      showText = normalizePayout(showText);

      // Classify row based on payout presence
      const hasWin = !!winText;
      const hasPlace = !!placeText;
      const hasShow = !!showText;

      // Clean runner name: remove speed figs like (107*), (98*)
      let horse = runnerText.replace(/\s+/g, " ").trim();
      // Remove speed figs: (107*), (98*), etc.
      horse = horse.replace(/\s*\(\d+\*?\)\s*$/g, "").trim();
      // Remove leading numbers (program numbers)
      horse = horse.replace(/^\d+\s*/, "").trim();
      // Remove jockey names in parentheses
      ...
      // Validate horse name
      if (!looksLikeHorseName(horse)) return;

      // Normalize horse name
      const normalizedHorse = normalizeHorseName(horse);
      if (!normalizedHorse) return;

      // Classify and assign (take FIRST match in table order)
      if (!winHorse && hasWin && hasPlace && hasShow) {
        winHorse = normalizedHorse;
        return; // Continue to find place/show
      }

      if (!placeHorse && !hasWin && hasPlace && hasShow) {
        placeHorse = normalizedHorse;
        return; // Continue to find show
      }

      if (!showHorse && !hasWin && !hasPlace && hasShow) {
        showHorse = normalizedHorse;
        return; // Continue (in case we haven't found win/place yet)
      }
    });

    // Set outcome (allow partial - at least one position is enough)
...
// ---------------------------------------------------------------------------
// NEW SCORING LOGIC — Hard-ban entries pages, force results pages to win
// ---------------------------------------------------------------------------
...
function scoreCSEResult(result, opts = {}) {
  ...
  // === HARD BAN: Equibase/HRN ENTRIES PAGES ===
  // If the page includes "/entries/" anywhere, it cannot possibly contain results.
  // These must be scored extremely low so they NEVER win unless absolutely nothing else is available.
  if (url.includes("/entries/") || title.includes("entries for") || snippet.includes("entries for")) {
    return -9999;   // <- completely banned unless it's the ONLY page
  }

  // === SOFT BAN: "entries-results" hybrid pages ===
  // These sometimes contain results, but usually not in usable format.
  if (url.includes("entries-results") || title.includes("entries-results")) {
    return -500;
  }

  // === STRONG BOOST: Real results pages ===
  let score = 0;

  const isResults =
    url.includes("/results/") ||
    url.includes("race-results") ||
    url.includes("/chart/") ||
    title.includes("results") ||
    title.includes("chart");

  if (isResults) score += 500;

  // === Host preference ===
  if (url.includes("equibase.com")) score += 400;
  if (url.includes("horseracingnation.com")) score += 250;

  // === Track match ===
  if (opts.targetTrackNormalized && title.includes(opts.targetTrackNormalized.toLowerCase())) {
    score += 60;
  }

  // === Race number match ===
  if (opts.targetRaceNo) {
    if (title.includes(`race ${opts.targetRaceNo}`) ||
        snippet.includes(`race ${opts.targetRaceNo}`)) {
      score += 60;
    }
  }
  ...
  if (opts.dateVariants) {
    let dateHit = false;
    for (const dv of opts.dateVariants) {
      if (title.includes(dv.toLowerCase()) || snippet.includes(dv.toLowerCase()) || url.includes(dv.toLowerCase())) {
        dateHit = true;
        score += 80;
        break;
      }
  ...
  // === Junk penalties ===
  const junkPatterns = [
    "picks", "handicapping", "analysis", "preview",
    "expert picks", "betting tips", "odds", "probabilities",
    "stats", "percent", "field size"
  ];

  for (const pat of junkPatterns) {
    if (title.includes(pat) || snippet.includes(pat)) score -= 80;
  }
...
export async function runFullVerifyRace(context) {
  ...
  try {
    ...
    if (bestResult) {
      ...
      const sourceLabel = bestResult.source === "equibase" ? "equibase-cse" : 
                         bestResult.source === "hrn" ? "hrn-cse" : 
                         bestResult.source || "cse";
