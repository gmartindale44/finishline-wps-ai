...
function scoreCSEResult(result, opts = {}) {
  ...
  if (link.includes("/entries/") || title.includes("entries for")) {
    return -9999;
  }

  // 🔥 HARD BOOST HRN results pages
  if (link.includes("horseracingnation.com/entries-results") ||
      link.includes("horseracingnation.com/results") ||
  ...
  if (link.includes("/results/") || link.includes("race-results") || link.includes("/chart/")) {
    score += 600;
  }

  if (title.includes("results") || snippet.includes("results")) {
    score += 200;
  }
  ...
  if (targetTrackNormalized) {
    ...
    if (
      title.includes(trackNeedle) ||
      snippet.includes(trackNeedle) ||
      link.includes(trackNeedle.replace(/\s+/g, "-"))
    ) {
      score += 60;
    }
  }
  ...
  if (targetRaceNo != null) {
    ...
    if (
      raceNeedles.some(
        (needle) => title.includes(needle) || snippet.includes(needle)
      )
    ) {
      score += 60;
    }
  ...
  if (
    junkPatterns.some(
      (p) => title.includes(p) || snippet.includes(p)
    )
  ) {
    score -= 200;
  }
