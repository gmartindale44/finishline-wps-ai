...
function isWpsTable($, table) {
  const headers = norm($(table).find("tr").first().text());
  return (
...
function findRaceScopedTable($, ctx) {
  const raceNo = Number(ctx?.race || ctx?.raceNumber || ctx?.raceNo);
  const track = norm(ctx?.track || "");

  if (!raceNo) return null;

  // 1. find "Race 1", "Race #1", "Race 1 –", etc.
  const racePattern = new RegExp(`race\\s*#?${raceNo}\\b`, "i");
  ...
  $("h1,h2,h3,h4,h5,h6,strong,b,p,div").each((i, el) => {
    ...
    if (racePattern.test(text)) {
      // Optional track match improves accuracy
      if (!track || text.includes(track)) {
        startNode = el;
        return false;
      }
    }
  ...
  // 2. Walk forward until first WPS table
  const nextTables = $(startNode).nextAll("table");
  for (let i = 0; i < nextTables.length; i++) {
    const tbl = nextTables[i];
    if (isWpsTable($, tbl)) return $(tbl);
  }
...
function parseHRNRaceOutcome(html, ctx) {
  ...
  let table = findRaceScopedTable($, ctx);

  // fallback: ANY WPS table (only if race-scoped fails)
  if (!table) {
    $("table").each((i, t) => {
      if (!table && isWpsTable($, t)) table = $(t);
    });
  }
  ...
  // extract first three runners
  const rows = table.find("tr").slice(1);
  const names = [];
  rows.each((i, row) => {
    const cols = $(row).find("td");
    if (cols.length > 0) {
      const raw = norm(cols.first().text());
      if (raw) names.push(raw.replace(/\(.*?\)/g, "").trim());
    }
  });
