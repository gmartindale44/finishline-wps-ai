 ...
 * Strips speed figs like (131*) or trailing numbers in parentheses
 * @param {string} name
 * @returns {string}
 */
function normalizeHorseName(name) {
  if (!name) return "";
  let normalized = String(name).toLowerCase().replace(/\s+/g, " ").trim();
...
function parseHRNRaceOutcome($, raceNo, track = "") {
  ...
  try {
    ...
    if (!winRow || !placeRow || !showRow) {
      console.warn("[verify_race_full] HRN missing finishers", {
        raceNo: requestedRaceNo,
        found: rows.map((r) => r.fin),
      });
      return outcome;
    }

    // Validate and set outcome - allow partial outcomes
    ...
    // Return if at least one position is valid (partial outcomes allowed)
    return outcome;
  } catch (error) {
...
function buildCSEQueries(track, date, raceNo) {
  ...
  if (dateParts.length === 3) {
    ...
    monthName = monthNames[parseInt(monthNum, 10)] || "";
  }
  ...
  if (monthName && day && year) {
    queries.push(`${track}${racePart} ${monthName} ${day} ${year} results Win Place Show order ${siteBias}`);
    queries.push(`${track}${racePart} ${monthName} ${day}, ${year} results Win Place Show ${siteBias}`);
  }
  
  if (monthNum && day && year) {
    queries.push(`${track}${racePart} ${monthNum}/${day}/${year} results Win Place Show ${siteBias}`);
  }
  
  queries.push(`${track}${racePart} ${date} results Win Place Show order ${siteBias}`);
  queries.push(`${track}${racePart} ${date} result chart official ${siteBias}`);
  queries.push(`${track} ${date} race ${raceNo} results equibase`);
  queries.push(`${track} ${date} race ${raceNo} results horseracingnation`);
...
function scoreCSEResult(result, targetDate, targetRaceNo, targetTrack) {
  ...
  if (isEquibase) {
    score += 400;
    if (url.includes("chart")) score += 50; // Extra boost for chart pages
  } else if (isHRN) {
    score += 200;
  ...
  if (targetDate) {
    ...
    const dateVariants = [
      dateStr, // 2025-11-22
      dateStr.replace(/-/g, ""), // 20251122
    ];
    
    const parts = dateStr.split("-");
    if (parts.length === 3) {
      dateVariants.push(`${parts[1]}/${parts[2]}/${parts[0]}`); // 11/22/2025
      const monthNames = ["", "jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"];
      const monthName = monthNames[parseInt(parts[1], 10)] || "";
      if (monthName) {
        dateVariants.push(`${monthName} ${parts[2]}, ${parts[0]}`); // nov 22, 2025
        dateVariants.push(`${monthName} ${parts[2]} ${parts[0]}`); // nov 22 2025
      }
    }
  ...
  // Penalize picks/tips/stats pages
  if (/power\s*picks?|picks?\s*(are|is)\s+winning|handicap|tips|stats/i.test(url) ||
      /power\s*picks?|picks?\s*(are|is)\s+winning|handicap|tips|stats/i.test(title)) {
    score -= 150;
  }
 ...
 * @param {string} date - ISO date (YYYY-MM-DD)
 * @param {string} raceNo
 * @returns {Promise<{ query: string; equibaseTop?: { title, link, score }; hrnTop?: { title, link, score }; googleUrl: string }>}
 */
async function searchRaceLinks(req, track, date, raceNo) {
  ...
  const equibaseTop = equibaseCandidates.length > 0 ? {
    title: equibaseCandidates[0].title,
    link: equibaseCandidates[0].link,
    score: equibaseCandidates[0].score,
  } : undefined;
  
  const hrnTop = hrnCandidates.length > 0 ? {
    title: hrnCandidates[0].title,
    link: hrnCandidates[0].link,
    score: hrnCandidates[0].score,
  } : undefined;
...
export async function runFullVerifyRace(context) {
  ...
  try {
    ...
    const { query: cseQuery, equibaseTop, hrnTop, googleUrl } = searchResult;

    // EQUIBASE-FIRST: Try Equibase from CSE results
    if (equibaseTop && equibaseTop.link) {
      try {
        const equibaseResult = await parseEquibaseOutcomeFromUrl(equibaseTop.link, {
          track: safeTrack,
          date: safeDate,
          raceNo: safeRaceNo,
        });

        if (equibaseResult && hasAnyOutcomePosition(equibaseResult)) {
          // Equibase succeeded - compute hits and return immediately
          const outcome = {
            win: equibaseResult.win || "",
            place: equibaseResult.place || "",
            show: equibaseResult.show || "",
          };
          const equibaseUrl = equibaseResult.link;
        ...
        summaryLines.push(`Step: verify_race`);
          ...
          summaryLines.push(`Top Result: ${equibaseTop.title || "Equibase chart"} -> ${equibaseUrl}`);
          summaryLines.push(`Source: equibase`);
          ...
          return {
            ...
            query: cseQuery,
            top: {
              title: equibaseTop.title || "Equibase chart",
              link: equibaseUrl,
            },
            outcome,
        ...
        console.error("[verify_race] Equibase parse from CSE link failed", {
          error: equibaseError?.message || String(equibaseError),
          link: equibaseTop.link,
        });
      }
    }
    // HRN-SECOND: Try HRN from CSE results if Equibase failed
    if (hrnTop && hrnTop.link) {
      try {
        const hrnResult = await parseHRNOutcomeFromUrl(hrnTop.link, {
          track: safeTrack,
          date: safeDate,
          raceNo: safeRaceNo,
        });

        if (hrnResult && hasAnyOutcomePosition(hrnResult)) {
          const outcome = {
            win: hrnResult.win || "",
            place: hrnResult.place || "",
            show: hrnResult.show || "",
          };
          const hrnUrl = hrnResult.link;
          ...
          summaryLines.push(`Step: verify_race`);
          summaryLines.push(`Query: ${cseQuery}`);
          summaryLines.push(`Top Result: ${hrnTop.title || "HRN results"} -> ${hrnUrl}`);
          summaryLines.push(`Source: hrn`);
          ...
          return {
            ...
            query: cseQuery,
            top: {
              title: hrnTop.title || "HRN results",
              link: hrnUrl,
            },
            outcome,
            predicted: predictedSafe,
        ...
        console.error("[verify_race] HRN parse from CSE link failed", {
          error: hrnError?.message || String(hrnError),
          link: hrnTop.link,
        });
      }
    }
    ...
    const bestLink = equibaseTop?.link || hrnTop?.link || null;
    const bestTitle = equibaseTop?.title || hrnTop?.title || "Google search";
