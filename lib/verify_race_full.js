...
// ---------------------------------------------------------------------------
// NEW SCORING LOGIC (Option A) — Prefer Results Pages Strongly
// ---------------------------------------------------------------------------

function scoreCSEResult(result, opts = {}) {
  const { host, dateVariants, targetRaceNo, targetTrackNormalized } = opts;
  ...
  // --------------------------------------------
  // Host Preference (strong weighting)
  // --------------------------------------------
  if (host.includes("equibase.com")) score += 400;
  if (host.includes("horseracingnation.com")) score += 250;

  // --------------------------------------------
  // Date Matching (same-day = good, wrong day = very bad)
  // --------------------------------------------
  let matchedDate = false;
  for (const dv of dateVariants) {
    if (text.includes(dv.toLowerCase())) {
      score += 70;
      matchedDate = true;
    }
  }
  if (!matchedDate) score -= 50;

  // --------------------------------------------
  // RESULTS vs ENTRIES Heuristics
  // --------------------------------------------
  const isResults =
    text.includes("results") ||
    text.includes("race results") ||
    text.includes("chart") ||
  ...
  const isEntriesResultsHybrid =
    text.includes("entries-results") ||
    (isEntries && isResults);

  // Prefer true RESULTS pages strongly
  if (isResults && !isEntries) score += 150;

  // Penalize hybrid pages
  if (isEntriesResultsHybrid) score -= 120;

  // Penalize pure entries heavily
  if (isEntries && !isResults) score -= 180;

  // --------------------------------------------
  // Track Matching Weight
  // --------------------------------------------
  if (
    targetTrackNormalized &&
    text.includes(targetTrackNormalized.toLowerCase())
  ) {
    score += 60;
  }
  ...
  // --------------------------------------------
  if (targetRaceNo && text.includes(`race ${targetRaceNo}`)) {
    score += 60;
  }
