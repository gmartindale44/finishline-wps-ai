...
/**
 * Returns true if the given URL/title/snippet text clearly contains the target race date.
 * Uses only string operations to avoid timezone issues.
 * @param {string} haystack - Combined text from link + title + snippet
 * @param {string} targetDateIso - Target date in ISO format (YYYY-MM-DD)
 * @param {string[]} dateVariants - Additional date variant strings to check
 * @returns {boolean} - True if the page contains the target date, false otherwise
 */
function pageContainsTargetDate(haystack, targetDateIso, dateVariants = []) {
  if (!targetDateIso || typeof targetDateIso !== "string") {
    // If we don't know the target date, don't filter.
    return true;
  }

  const trimmed = targetDateIso.trim();
  ...
  if (!yyyy || Number.isNaN(m) || Number.isNaN(d)) return true;
...
function parseHRNRaceOutcome($, ctx = {}) {
  // Find the WPS payout table:
  // header must contain something like: "Runner", "Win", "Place", "Show"
  let wpsTable = null;
  ...
  $("table").each((_, el) => {
    ...
    const hasRunner = headers.some((h) => h.includes("runner"));
    ...
    if (hasRunner && hasWin && hasPlace && hasShow) {
      wpsTable = $(el);
  ...
  // Helper to clean payout cells: "-" / empty => ""
  function cleanPayoutCell(text) {
    const t = (text || "").trim();
    if (!t || t === "-" || t === "—") return "";
  ...
  const headerRow = wpsTable.find("thead tr, tr").first();
  const headerCells = headerRow.find("th, td").toArray();
  const headerTexts = headerCells.map((cell) => $(cell).text().trim().toLowerCase());
  
  const runnerColIdx = headerTexts.findIndex((h) => h.includes("runner"));
  ...
  // If we can't find all required columns, bail out
  ...
  wpsTable.find("tbody tr, tr").slice(1).each((rowIndex, tr) => {
    const cells = $(tr).find("td, th").toArray();
    if (!cells.length) return;

    // Get runner from the runner column index
    const runnerRaw = cells[runnerColIdx] ? $(cells[runnerColIdx]).text().trim() : "";

    // Skip header-ish or note rows
    if (!runnerRaw) return;
    if (/prelim/i.test(runnerRaw)) return;

    // normalizeHorseName already strips things like "(107*)" and trailing asterisks
    const runnerName = normalizeHorseName(runnerRaw);
    if (!runnerName) return;

    // Get payouts from the correct column indices
    const winText = cleanPayoutCell(cells[winColIdx] ? $(cells[winColIdx]).text() : "");
    const placeText = cleanPayoutCell(cells[placeColIdx] ? $(cells[placeColIdx]).text() : "");
    const showText = cleanPayoutCell(cells[showColIdx] ? $(cells[showColIdx]).text() : "");

    // Ignore rows with no payouts at all
    if (!winText && !placeText && !showText) return;
  ...
  // Pick W/P/S by payout presence, in order:
  // 1) First row with Win
  // 2) First remaining row with Place
  // 3) First remaining row with Show
  function pickIndex(predicate, usedIndices) {
  ...
  const winIdx = pickIndex((r) => r.hasWin, used);
  if (winIdx !== null) used.add(winIdx);

  const placeIdx = pickIndex((r) => r.hasPlace, used);
  if (placeIdx !== null) used.add(placeIdx);

  const showIdx = pickIndex((r) => r.hasShow, used);

  const win = winIdx !== null ? rows[winIdx].name : null;
  const place = placeIdx !== null ? rows[placeIdx].name : null;
  const show = showIdx !== null ? rows[showIdx].name : null;
...
function scoreCSEResult(result, opts = {}) {
  ...
  // --- Hard date gate: if we know the target date and this page doesn't contain it, reject. ---
  if (targetDateIso) {
    const hasTargetDate = pageContainsTargetDate(haystack, targetDateIso, dateVariants);
    if (!hasTargetDate) {
      // Example: user asked for 2025-11-24, but this page is clearly 2025-01-24.
      // We never want to consider this page for scoring.
      return -5000;
    }
  }
  ...
  if (dateVariants && dateVariants.length) {
    ...
    if (!matchedDate) {
      score -= 50; // small penalty if date is clearly different
    }
  }
