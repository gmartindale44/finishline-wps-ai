...
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  // 1) Find the WPS table: headers must contain "Runner" and Win/Place/Show.
  let wpsTable = null;
  $("table").each((_, table) => {
    if (wpsTable) return;
    const headers = $(table)
    ...
    const hasWin = headers.some((h) => h === "win");
    const hasPlace = headers.some((h) => h === "place");
    const hasShow = headers.some((h) => h === "show");
  ...
  /** @type {{horse: string, hasWin: boolean, hasPlace: boolean, hasShow: boolean}[]} */
  const horses = [];
  ...
  rows.each((_, tr) => {
    ...
    const rawHorse = normalizeHorseName($(cells[idxRunner]).text());
    if (!rawHorse) return;

    const winCell = cells[idxWin] ? $(cells[idxWin]).text().trim() : "";
    const placeCell = cells[idxPlace] ? $(cells[idxPlace]).text().trim() : "";
    const showCell = cells[idxShow] ? $(cells[idxShow]).text().trim() : "";

    const hasWin = !!normalizePayout(winCell);
    const hasPlace = !!normalizePayout(placeCell);
    const hasShow = !!normalizePayout(showCell);

    // Ignore completely empty payout rows.
    if (!hasWin && !hasPlace && !hasShow) return;
  ...
  // Pass 1: strict interpretation of HRN pattern.
  for (const h of horses) {
    const { horse, hasWin, hasPlace, hasShow } = h;
    ...
    // PLACE: row with P+S only (no Win)
    if (!placeHorse && !hasWin && hasPlace && hasShow) {
      placeHorse = horse;
      continue;
    }

    // --- SHOW HORSE (3rd place) ---
    // After identifying Win and Place, continue scanning to pick up
    // the next row containing a Show payout (even if Win/Place cells are empty)
    if (!showHorse && hasShow) {
      showHorse = horse;
      continue;
    }
  }

  // Pass 2: fallbacks in case some patterns are missing.
  // - If no winHorse, take the first row that hasWin.
  if (!winHorse) {
    const candidate = horses.find((h) => h.hasWin);
    if (candidate) winHorse = candidate.horse;
  }

  // - If no placeHorse, take the first row (excluding win) that hasPlace.
  if (!placeHorse) {
    const candidate = horses.find(
      (h) => h.hasPlace && h.horse !== winHorse
    );
    if (candidate) placeHorse = candidate.horse;
  }

  // - If no showHorse, take the first row (excluding win/place) that hasShow.
  if (!showHorse) {
    const candidate = horses.find(
      (h) => h.hasShow && h.horse !== winHorse && h.horse !== placeHorse
    );
    if (candidate) showHorse = candidate.horse;
  }

  // Final fallback: if show is still missing, try the next horse
  // after win/place in reading order.
  if (!showHorse) {
    const ordered = rows
      .map((_, tr) => {
        const cells = $(tr).find("td, th");
        const runnerText = cells.eq(idxRunner).text();
        return normalizeHorseName(runnerText);
      })
      .filter(Boolean);

    // Remove win and place already identified
    const remaining = ordered.filter(
      (h) => h !== winHorse && h !== placeHorse
    );

    if (remaining.length > 0) {
      showHorse = remaining[0];
    }
  }
