...
function parseHRNRaceOutcome($, raceNo) {
  ...
  try {
    // Remove all script and style tags from the document before parsing
    $("script, style").remove();
...
function pickBestResult(results) {
  try {
    ...
    // 1) Prefer Equibase chart pages FIRST
    ...
    // 2) Fall back to HorseracingNation entries/results pages
  ...
  try {
    ...
    try {
      ...
      if (equibaseResult && equibaseResult.outcome && isValidOutcome(equibaseResult.outcome)) {
        ...
        const hitParts = [];
        if (hits.winHit) hitParts.push("Win");
        if (hits.placeHit) hitParts.push("Place");
        if (hits.showHit) hitParts.push("Show");
        if (hits.top3Hit) hitParts.push("Top3");
