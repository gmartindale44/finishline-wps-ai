...
function findRaceWpsTable($, ctx) {
  const raceNo =
    Number(ctx?.race ?? ctx?.raceNo ?? ctx?.raceNumber ?? NaN) || null;
  const trackName = norm(ctx?.track ?? ctx?.trackName ?? ctx?.track_label ?? "");

  // 1) Find all candidate tables that look like Runner (Speed) WPS tables
  const tables = [];
  $("table").each((_, el) => {
    const $table = $(el);
    const headerText = norm($table.find("tr").first().text());
    if (
  ...
  // 2) Score each table by nearby headings: race number + track
  let best = null;
  ...
  for (const $table of tables) {
    ...
    // Walk backwards through siblings to find headings/labels
    let $node = $table;
    for (let i = 0; i < 20; i += 1) {
      ...
      if (trackName && text.includes(trackName)) {
        score += 10;
      }
    }

    // Slight preference for earlier tables if scores tie
    score += -0.001 * tables.indexOf($table);
...
export function parseHRNRaceOutcome(html, ctx) {
  ...
  const wpsTable = findRaceWpsTable($, ctx);
  if (!wpsTable || !wpsTable.length) {
    return { win: "", place: "", show: "" };
  }

  // Get all data rows after the header row
...
export async function runFullVerifyRace(context) {
  ...
  // Treat date from API as already canonical - no mutation
  ...
  if (!canonicalDateIso || !/^\d{4}-\d{2}-\d{2}$/.test(canonicalDateIso)) {
    return {
      ok: false,
      step: "verify_race_full_error",
      error: "Missing or invalid canonical date",
      summary: "Race date is required but was not provided or is invalid.",
      outcome: { win: "", place: "", show: "" },
    };
  }
  
  const finalDate = canonicalDateIso; // single source of truth
  
  // Temporary debug log
  console.log("DEBUG_CANONICAL_DATE", {
    fromContext: { date: context?.date, raceDate: context?.raceDate },
    canonicalDateIso,
  });
