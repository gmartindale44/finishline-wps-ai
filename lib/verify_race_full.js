...
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  rows.each((i, row) => {
    ...
    const winText = cleanPayoutText($cells.eq(idxWin).text());
    const placeText = cleanPayoutText($cells.eq(idxPlace).text());
    const showText = cleanPayoutText($cells.eq(idxShow).text());
    ...
    // 3) SHOW = first horse with a Show payout that isn't Win or Place
    //    This is the key change that lets us pick the 3rd horse (e.g. Frosted Lace).
    if (
      !showHorse &&
      hasShow &&
      horseName !== winHorse &&
      horseName !== placeHorse
    ) {
      showHorse = horseName;
    }
  });
