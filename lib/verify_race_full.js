...
/**
 * Very simple HRN parser:
 * - Find the first table whose header row contains "win", "place", and "show".
 * - Take the first three runner names from that table in order.
 * - Map them to { win, place, show }.
 *
 * Keep the existing function signature exactly the same as before.
 */
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  // 1) Find the first WPS-looking table on the page.
  let wpsTable = null;
  $("table").each((_, table) => {
    if (wpsTable) return; // already found one
    const $table = $(table);
    const headerText = $table.find("tr").first().text().toLowerCase();

    // Basic W/P/S header detection.
    if (
  ...
  if (!wpsTable) {
    // No suitable table found, signal "no outcome" so caller can fall back.
    return { win: null, place: null, show: null };
  }

  // 2) Collect the first three runner names from the rows under the header.
  const horses = [];
  const rows = wpsTable.find("tr").slice(1); // skip header row

  rows.each((_, row) => {
    if (horses.length >= 3) return false; // stop once we have 3
  ...
  const [winHorse, placeHorse, showHorse] = horses;
