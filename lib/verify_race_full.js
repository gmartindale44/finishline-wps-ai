...
function formatDateVariants(dateStr) {
  ...
  return { iso: dateStr, humanShort, usNumeric };
}
...
function buildCSEQueries(track, date, raceNo) {
  ...
  const dateVariants = formatDateVariants(date);
  
  const siteBias = "(site:equibase.com OR site:horseracingnation.com OR site:entries.horseracingnation.com)";
  
  // Query variations using the canonical date and its variants
  queries.push(`${track}${racePart} ${dateVariants.humanShort} results Win Place Show order ${siteBias}`);
  queries.push(`${track}${racePart} ${dateVariants.humanShort.replace(/(\d+) (\d+) (\d+)/, "$1 $2, $3")} results Win Place Show ${siteBias}`);
...
export async function runFullVerifyRace(context) {
  ...
  // Treat context.date as canonical - it should already be normalized by handler
  ...
  // Only normalize if date is missing or not in ISO format
