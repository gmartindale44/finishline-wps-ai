...
function extractRaceNumberFromHeading(text) {
  const norm = normText(text);
  if (!norm) return null;
  // Common patterns:
  // "parx racing race # 2, 11:32 am"
  // "delta downs race 1, 12:55 p.m."
  const m =
    norm.match(/race\s*#\s*(\d+)/i) ||
    norm.match(/race\s+(\d+)/i);
  if (!m) return null;
  const n = Number(m[1]);
...
function findRaceSections($) {
  // Race headings are usually anchors or headings near the top of each block.
  const selectors = [
    "a",
    "h1",
    "h2",
  ...
  const sections = [];
  $(selectors).each((_, el) => {
    const raceNo = extractRaceNumberFromHeading($(el).text());
    if (raceNo != null) {
      sections.push({ raceNo, el });
    }
  });
  return sections;
}

function findWpsTableForRace($, requestedRaceNo) {
  // Collect all candidate WPS tables up front
  const allWpsTables = [];
  $("table").each((_, el) => {
    if (isWpsTable($, el)) {
      allWpsTables.push($(el));
    }
  });

  if (allWpsTables.length === 0) {
    return null;
  }

  // If no race number is provided, keep existing behavior:
  // just use the first WPS table on the page.
  if (!requestedRaceNo || !Number.isFinite(requestedRaceNo)) {
    return allWpsTables[0];
  }

  const sections = findRaceSections($);
  if (sections.length === 0) {
    // No detectable headings; fallback to first table.
    return allWpsTables[0];
  }

  // Find the section for the requested race, fallback to first section
  let targetSection = sections.find(s => s.raceNo === requestedRaceNo);
  if (!targetSection) {
    targetSection = sections[0];
  }

  const targetRaceNo = targetSection.raceNo;

  // Walk forward from the target section until the next race heading,
  // and pick the first WPS table in that slice.
  let $cursor = $(targetSection.el).next();
  while ($cursor && $cursor.length) {
    // If we hit another race heading, stop scanning this section.
    const maybeRaceNo = extractRaceNumberFromHeading($cursor.text());
    if (maybeRaceNo != null && maybeRaceNo !== targetRaceNo) {
      break;
    }

    if ($cursor.is("table") && isWpsTable($, $cursor[0])) {
      return $cursor;
    }
  ...
  // Section scan failed; as a secondary heuristic, attach the nearest
  // previous heading to each WPS table and pick best match.
  function headingRaceForTable($table) {
    let $h = $table.prev();
    while ($h && $h.length) {
      const raceNo = extractRaceNumberFromHeading($h.text());
      if (raceNo != null) return raceNo;
      $h = $h.prev();
    }
    return null;
  }

  let best = allWpsTables[0];
  let bestScore = Number.POSITIVE_INFINITY;
  for (const $t of allWpsTables) {
    const rn = headingRaceForTable($t);
    if (rn == null) continue;
    const score = Math.abs(rn - requestedRaceNo);
    if (score < bestScore) {
      bestScore = score;
      best = $t;
    }
...
export function parseHRNRaceOutcome(html, ctx) {
  ...
  const requestedRaceNoRaw =
    (ctx && (ctx.raceNo ?? ctx.race ?? ctx.raceNumber)) ?? null;
  const requestedRaceNo = requestedRaceNoRaw != null
    ? Number(requestedRaceNoRaw)
    : null;

  const wpsTable = findWpsTableForRace($, requestedRaceNo);
...
export async function runFullVerifyRace(context) {
  ...
  const buildSummary = ({ usingDate, step, source, outcome, predicted, hits, topLink, track, raceNo, query, uiDateRaw }) => {
    const lines = [];
    if (uiDateRaw) {
      lines.push(`UI date raw: ${uiDateRaw}`);
    }
