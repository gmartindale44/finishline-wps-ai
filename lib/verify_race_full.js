...
// Simple HRN WPS table finder: first table whose header contains Win/Place/Show
function findWpsTable($) {
  const tables = $("table");
  ...
  for (let i = 0; i < tables.length; i++) {
    ...
    // We only care that this table has Win / Place / Show columns.
    if (
...
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  // --- Table selection: use simplest possible WPS detection ---
  const wpsTable = findWpsTable($);

  if (!wpsTable || wpsTable.length === 0) {
    // Could not find a WPS table; signal no outcome
    return { win: "", place: "", show: "" };
  }

  // All subsequent parsing should operate on this table
