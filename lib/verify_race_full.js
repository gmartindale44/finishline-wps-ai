 ...
 * @param {Object} opts - { host, dateVariants, targetRaceNo, targetTrackNormalized }
...
function scoreCSEResult(result, opts = {}) {
  ...
  // --- 1. Host preference (keep existing behavior) ---
  // Equibase first, then HRN, then everything else.
  if (host.includes("equibase.com")) {
    score += 400;
  } else if (
    host.includes("horseracingnation.com") ||
    host.includes("entries.horseracingnation.com")
  ) {
    score += 200;
  }
  
  // --- 2. Date / race / track matching (existing behavior should remain) ---
  // NOTE: We assume opts already has targetDateIso, targetRaceNo, targetTrack.
  const text = `${url} ${title} ${snippet}`;
  if (opts.dateVariants) {
    for (const d of opts.dateVariants) {
      if (!d) continue;
      if (text.includes(d.toLowerCase())) {
        score += 70; // date match bonus
        break;
      }
    }
  }
  
  if (opts.targetRaceNo != null) {
    const racePattern = new RegExp(`race\\s*#?\\s*${opts.targetRaceNo}\\b`, "i");
    if (racePattern.test(text)) {
      score += 50;
    }
  }
  
  if (opts.targetTrackNormalized) {
    const trackNorm = opts.targetTrackNormalized.toLowerCase();
    if (text.includes(trackNorm)) {
      score += 40;
    }
  }
  
  // --- 3. RESULTS vs ENTRIES heuristics (NEW) ---
  // We want to strongly prefer true results / chart pages and penalize entries pages.
  const hasResultsKeyword =
    /results?|race_results|race[-_\s]?result|full[-_\s]?chart|full[-_\s]?result|chart\b/.test(
      text
    );
  const isEntriesResults = /entries-results/.test(url);
  const isEntriesOnly =
    /\/entries(\/|$)/.test(url) ||
    /\bentries\b/.test(title) ||
    /\bovernight\b/.test(title);
  
  if (hasResultsKeyword) {
    // Real results / chart pages get a strong boost.
    score += 150;
  }
  
  if (isEntriesResults) {
    // Hybrid "entries-results" pages are usually NOT great for WPS parsing.
    // Penalize heavily so true results pages win unless nothing else is available.
    score -= 120;
  } else if (isEntriesOnly) {
    // Plain entries / overnight pages are even worse for our parser.
    score -= 180;
  }
  
  // --- 4. Junk / tips / picks penalties (keep and extend existing behavior) ---
  const junkPatterns = [
    "picks are winning",
    "picks is winning",
    "handicapping picks",
  ...
  for (const phrase of junkPatterns) {
    if (text.includes(phrase)) {
      score -= 150;
    }
  }
  
  // Generic "stats / payouts" style junk (often not raw result charts).
  if (
    /\bpicks\b/.test(text) ||
    /\bstats\b/.test(text) ||
  ...
  // Avoid obvious noise like PDFs or non-HTML docs.
  if (url.endsWith(".pdf") || url.endsWith(".csv") || url.endsWith(".xlsx")) {
    score -= 200;
  }
