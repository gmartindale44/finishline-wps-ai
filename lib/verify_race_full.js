...
function isWpsTable($, table) {
  const header = normalize($(table).find("tr").first().text());
  if (!header.includes("win")) return false;
  if (!header.includes("place")) return false;
  if (!header.includes("show")) return false;
  return true;
}
...
function findRaceScopedWpsTable($, raceNumber) {
  ...
  const racePatterns = [
    ...
    `race ${raceNumber} -`,
    `race ${raceNumber}–`,
    `race ${raceNumber} results`,
  ];

  const headings = $("h1, h2, h3, h4, h5, h6, strong, b, p, div");

  let startIndex = -1;

  headings.each((i, el) => {
    const t = normalize($(el).text());
    if (racePatterns.some(p => t.includes(p))) {
      startIndex = i;
      return false;
    }
  });
  ...
  // Scan forward until next race heading or table found
  for (let i = startIndex + 1; i < headings.length; i++) {
    const el = headings[i];
    const text = normalize($(el).text());

    // Stop scanning when the next race heading is reached
    if (text.match(/race\s+\d+/) && !text.includes(`race ${raceNumber}`)) {
      break;
    }

    // Did we land on a table?
    const nextNode = $(el).nextAll("table").first();
    if (nextNode.length > 0 && isWpsTable($, nextNode)) {
      return nextNode;
    }
  }
...
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  const raceNumber =
    ctx?.race ?? ctx?.raceNumber ?? ctx?.raceNo ?? null;

  let wpsTable =
    findRaceScopedWpsTable($, raceNumber) ||
    findAnyWpsTable($);

  if (!wpsTable || wpsTable.length === 0) {
    return { win: "", place: "", show: "" };
  }
...
export async function runFullVerifyRace(context) {
  ...
  try {
    ...
    // CRITICAL: Use canonical date from context - never compute or increment dates
    const canonicalDateIso =
      context?.date ??
      context?.raceDate ??
      safeDate ??
      null;

    // If we somehow don't have a canonical date, log warning but use safeDate as last resort
    const fallbackDate = canonicalDateIso || safeDate || "";
    if (!canonicalDateIso && safeDate) {
      console.warn("[verify_race_full_fallback] Using safeDate as fallback - canonical date missing", {
    ...
    // The cseQuery should already use safeDate, but double-check it contains the correct date
    ...
    return {
      ...
      query: finalQuery,
      date: fallbackDate, // Ensure response.date uses canonical date
      top: bestLink ? {
        title: bestTitle,
        link: bestLink,
      } : {
