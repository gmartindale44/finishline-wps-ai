...
// Race-scoped table detection ensures we select the correct WPS table for the target race.
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  function headerHasWps(text) {
    const t = lower(text);
    return t.includes("win") && t.includes("place") && t.includes("show");
  }
  ...
  function findRaceScopedWpsTable() {
    ...
    // Find the heading/paragraph that mentions "Race <n>"
    const raceHeading = $("h1,h2,h3,h4,h5,h6,p,div")
    ...
    raceHeading.nextAll("table").each((_, table) => {
      ...
      const headerLower = lower(headerText);

      // Explicitly ignore "Runner (Speed)" tables.
      if (headerLower.includes("speed") && !headerLower.includes("win")) return;
  ...
  // ---------- 2) Work out column indices (Runner / Win / Place / Show) ----------
  ...
  headerCells.each((i, cell) => {
    ...
    if (text.includes("runner")) idxRunner = i;
    if (text.startsWith("win")) idxWin = i;
    if (text.startsWith("place")) idxPlace = i;
    if (text.startsWith("show")) idxShow = i;
  });

  // Fallback if headers are a little weird: assume classic layout.
  if (idxWin === -1 || idxPlace === -1 || idxShow === -1) {
    idxRunner = 0;
    idxWin = 1;
    idxPlace = 2;
    idxShow = 3;
  ...
  // ---------- 3) First pass: use actual payouts where possible ----------
  bodyRows.each((_, row) => {
    const cells = $(row).find("td");
    if (!cells.length) return;

    const rawName = $(cells.get(idxRunner) || "").text();
    const horse = normalizeHorseName(rawName);
    if (!horse) return;

    orderedHorses.push(horse);

    const winTxt = $(cells.get(idxWin) || "").text();
    const placeTxt = $(cells.get(idxPlace) || "").text();
    const showTxt = $(cells.get(idxShow) || "").text();

    if (!winHorse && hasNumericPayout(winTxt)) {
      winHorse = horse;
      // don't return; we still want to track later rows for place/show
    }

    if (!placeHorse && hasNumericPayout(placeTxt) && horse !== winHorse) {
      placeHorse = horse;
    }

    if (!showHorse && hasNumericPayout(showTxt) && horse !== winHorse && horse !== placeHorse) {
      showHorse = horse;
    }
  });

  // ---------- 4) Ultra-simple reading-order fallback for Show ----------
  // If Show is still null, just "go down one more horse" for this race.
  if (!showHorse && orderedHorses.length) {
    const lowerName = (s) => (s || "").toString().toLowerCase();

    const findIndex = (name) =>
      orderedHorses.findIndex((h) => lowerName(h) === lowerName(name));

    let startIdx = -1;
    if (placeHorse) {
      startIdx = findIndex(placeHorse);
    } else if (winHorse) {
      startIdx = findIndex(winHorse);
    }

    if (startIdx !== -1) {
      for (let i = startIdx + 1; i < orderedHorses.length; i++) {
        const candidate = orderedHorses[i];
        if (
          lowerName(candidate) !== lowerName(winHorse) &&
          lowerName(candidate) !== lowerName(placeHorse)
        ) {
          showHorse = candidate;
          break;
        }
