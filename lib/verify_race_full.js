...
export async function runFullVerifyRace(context) {
  ...
  // Use context.date as-is - handler already normalized it
  // DO NOT re-normalize if already YYYY-MM-DD
  const rawDate = context.date || "";
  let targetDateIso = "";
  
  if (/^\d{4}-\d{2}-\d{2}$/.test(rawDate)) {
    // Already ISO format - use as-is
    targetDateIso = rawDate;
  } else if (rawDate) {
    // Optional: support MM/DD/YYYY, but don't call new Date()
  ...
  // Only fall back to today if no date provided
  if (!targetDateIso) {
    const now = new Date();
    const year = now.getUTCFullYear();
    const month = String(now.getUTCMonth() + 1).padStart(2, "0");
    ...
    console.log("[runFullVerifyRace] Using date from context:", targetDateIso);
  }
