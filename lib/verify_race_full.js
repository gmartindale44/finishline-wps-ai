...
function extractRaceNoFromText(txt) {
  if (!txt) return null;
  const m = normText(txt).match(/\brace\s*#?\s*(\d+)\b/);
  if (!m) return null;
  const n = parseInt(m[1], 10);
  return Number.isNaN(n) ? null : n;
}

function isHeadingCandidate(node) {
  if (!node || !node.tagName) return false;
  const tag = String(node.tagName).toLowerCase();
  // HRN uses links + text blocks around the race header
  return [
    "h1",
    "h2",
    "h3",
...
// Walk backwards from a WPS table to find the nearest "Race # N" heading.
function annotateWpsTables($) {
  const annotated = [];
  ...
  $("table").each((idx, el) => {
    ...
    // Walk up the DOM tree, and for each level, walk previous siblings.
    while (current.length && raceNo == null && guard < 50) {
      let prev = current.prev();

      while (prev.length && raceNo == null && guard < 200) {
        const node = prev.get(0);

        if (isHeadingCandidate(node)) {
          const headingText = $(prev).text();
          const candidate = extractRaceNoFromText(headingText);
          if (candidate != null) {
            raceNo = candidate;
            break;
          }
      ...
      // Move one level up if we still haven't found a heading.
...
function chooseWpsTableForRace($, requestedRaceNoRaw) {
  const annotated = annotateWpsTables($);

  if (!annotated.length) {
    return null;
  }

  let requestedRaceNo = null;
  if (requestedRaceNoRaw != null) {
    const n = parseInt(String(requestedRaceNoRaw), 10);
    if (!Number.isNaN(n)) requestedRaceNo = n;
  }

  // Prefer exact race number match, if we have one.
  if (requestedRaceNo != null) {
  ...
  // Fallback: first WPS table on the page (this preserves existing
  // successful behaviour for Race 1 and single-race cards).
...
export function parseHRNRaceOutcome(html, ctx) {
  ...
  const requestedRaceNo =
    (ctx && (ctx.raceNo ?? ctx.race ?? ctx.raceNumber)) ?? null;

  const wpsTable = chooseWpsTableForRace($, requestedRaceNo);
...
export async function runFullVerifyRace(context) {
  ...
  // Use the canonical date from context as-is (already normalized by API handler)
  const canonicalDateIso =
    context?.canonicalDateIso ||
    context?.date ||
    context?.raceDate ||
    null;

  const uiDateRaw = context?.dateRaw || context?.date || null;

  if (!canonicalDateIso) {
    return {
      ok: false,
      step: "verify_race",
      error: "runFullVerifyRace: canonicalDateIso is required",
  ...
  const buildSummary = ({ usingDate, step, source, outcome, predicted, hits, topLink, track, raceNo, query, uiDateRaw }) => {
    const lines = [];
    if (uiDateRaw) {
    ...
    const win = (outcome && outcome.win) ? outcome.win.trim() : "-";
    const place = (outcome && outcome.place) ? outcome.place.trim() : "-";
    const show = (outcome && outcome.show) ? outcome.show.trim() : "-";
  ...
  try {
    ...
    if (hrnTop && hrnTop.link) {
      try {
        ...
        if (parseResult && isValidOutcome(parseResult)) {
          // CRITICAL: Clean outcome to only include win/place/show - never spread entire object which might contain ok
          const cleanHrnOutcome = {
            win: parseResult.win || "",
            place: parseResult.place || "",
            show: parseResult.show || "",
          };
          delete cleanHrnOutcome.ok; // Defensive cleanup
          bestResult = {
            outcome: cleanHrnOutcome,
            source: "hrn",
    ...
    if (!bestResult && equibaseTop && equibaseTop.link) {
      try {
        ...
        if (parseResult && isValidOutcome(parseResult)) {
          // CRITICAL: Clean outcome to only include win/place/show - never spread entire object which might contain ok
          const cleanEquibaseOutcome = {
            win: parseResult.win || "",
            place: parseResult.place || "",
            show: parseResult.show || "",
          };
          delete cleanEquibaseOutcome.ok; // Defensive cleanup
          bestResult = {
            outcome: cleanEquibaseOutcome,
            source: "equibase",
    ...
    if (!bestResult && safeRaceNo) {
      try {
        ...
        if (equibaseDirect && equibaseDirect.outcome && isValidOutcome(equibaseDirect.outcome)) {
          // CRITICAL: Clean outcome to only include win/place/show - never spread entire object which might contain ok
          const cleanEquibaseOutcome = {
            win: equibaseDirect.outcome.win || "",
            place: equibaseDirect.outcome.place || "",
            show: equibaseDirect.outcome.show || "",
          };
          delete cleanEquibaseOutcome.ok; // Defensive cleanup
          bestResult = {
            outcome: cleanEquibaseOutcome,
    ...
    if (bestResult) {
      ...
      // CRITICAL: Clean normalizedOutcome to ensure no ok property exists before using it
      const cleanNormalizedOutcome = {
        win: normalizedOutcome.win || null,
        place: normalizedOutcome.place || null,
        show: normalizedOutcome.show || null,
      };
      delete cleanNormalizedOutcome.ok; // Defensive cleanup
      
      const outcome = cleanNormalizedOutcome;
      const predictedSafe = {
        win: (predicted?.win || "").trim(),
        place: (predicted?.place || "").trim(),
        show: (predicted?.show || "").trim(),
      ...
      // CRITICAL: Recompute ok from cleaned outcome - NEVER hardcode ok:true
      ...
      return {
        ok: hasValidOutcome, // Recomputed from cleaned outcome - never hardcode true
        step: "verify_race",
        date: canonicalDateIso,
        track: safeTrack,
        raceNo: safeRaceNo,
        ...
        outcome: cleanNormalizedOutcome, // Use cleaned outcome with no ok property
        ...
        summary: buildSummary({
          ...
          step: "verify_race",
          source: sourceLabel,
          outcome: cleanNormalizedOutcome,
          predicted: predictedSafe,
          hits,
