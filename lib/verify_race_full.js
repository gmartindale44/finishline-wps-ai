...
export async function runFullVerifyRace(context) {
  ...
  try {
    ...
    // === STEP 1: Try Equibase direct first (without CSE) ===
    if (safeRaceNo) {
      ...
      if (equibaseDirect && equibaseDirect.outcome && equibaseDirect.outcome.win) {
        const outcome = equibaseDirect.outcome;
        const predictedSafe = {
          win: (predicted?.win || "").trim(),
          place: (predicted?.place || "").trim(),
          show: (predicted?.show || "").trim(),
        ...
        return {
          ...
          summary: buildSummary({
            ...
            predicted: predictedSafe,
            hits,
            topLink: equibaseDirect.url,
            track: safeTrack,
            raceNo: safeRaceNo,
          }),
          debug: {
            source: "equibase-direct",
            equibaseUrl: equibaseDirect.url,
          },
        };
      }
    }

    // === STEP 2: If Equibase direct failed, try CSE search ===
    const searchResult = await searchRaceLinks(req, safeTrack, safeDate, safeRaceNo);
    const { query: cseQuery, orderedCandidates, equibaseCandidates, hrnCandidates, bestOverall, googleUrl } = searchResult;
    
    // Check if CSE only found entries pages (no results pages)
    const hasResultsPages = orderedCandidates.some(c => {
    ...
    const onlyEntriesPages = !hasResultsPages && orderedCandidates.length > 0;
    ...
    } else if (orderedCandidates.length > 0) {
      parserNote = "Parser note: Equibase/HRN chart was fetched but Win/Place/Show horses could not be reliably parsed. Falling back to Google-only stub.";
    } else {
      parserNote = "Parser note: No Equibase/HRN results pages found for this race date. Falling back to Google-only stub.";
    }
    ...
    return {
      ...
      debug: {
        googleUrl,
        onlyEntriesPages,
        candidatesCount: orderedCandidates.length,
      },
    };
  } catch (err) {
