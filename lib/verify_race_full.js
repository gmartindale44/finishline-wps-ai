...
function scoreCSEResult(result, opts = {}) {
  ...
  // ---- Hard penalties for entries pages (future races / non-results) ----
  ...
  if (
    entriesPatterns.some(
      (p) => link.includes(p) || title.includes(p) || snippet.includes(p)
    )
  ) {
    // Make it almost impossible for entries pages to win when any results page exists
    score -= 2000;
  }
