 ...
 * STRICT: Only accepts canonical ISO format "YYYY-MM-DD" and transforms via pure string ops
 * @param {string} dateStr - ISO date string "YYYY-MM-DD" (must be canonical)
 * @returns {{ iso: string; humanShort: string; usNumeric: string }}
 */
function formatDateVariants(dateStr) {
  // Validate input is ISO format
  if (!dateStr || typeof dateStr !== "string") {
    console.warn("[formatDateVariants] Invalid input, returning as-is:", dateStr);
    return { iso: dateStr || "", humanShort: dateStr || "", usNumeric: dateStr || "" };
  }
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
    console.warn("[formatDateVariants] Not ISO format, returning as-is:", trimmed);
    return { iso: trimmed, humanShort: trimmed, usNumeric: trimmed };
  }
  
  // Pure string parsing - no Date objects
  const parts = parseISODateString(trimmed);
  if (!parts) {
    console.warn("[formatDateVariants] Failed to parse, returning as-is:", trimmed);
    return { iso: trimmed, humanShort: trimmed, usNumeric: trimmed };
  }
  ...
  if (month < 1 || month > 12 || day < 1 || day > 31) {
    console.warn("[formatDateVariants] Invalid month/day, returning as-is:", trimmed);
    return { iso: trimmed, humanShort: trimmed, usNumeric: trimmed };
  }
  
  // Pure string formatting - no Date objects, no timezone
  const humanShort = `${MONTH_NAMES_SHORT[month - 1]} ${day} ${year}`; // e.g. "Nov 22 2025"
  const usNumeric = `${month}/${day}/${year}`;                          // e.g. "11/22/2025"
  
  return { iso: trimmed, humanShort, usNumeric };
}
...
export function getCanonicalRaceDate(rawDate) {
  // DEBUG: Log input
  console.log('[getCanonicalRaceDate] Input rawDate:', rawDate, 'type:', typeof rawDate);
  ...
  console.log('[getCanonicalRaceDate] Trimmed:', trimmed);

  // If already in YYYY-MM-DD format, return as-is (STRICT: no modification)
  if (/^\d{4}-\d{2}-\d{2}$/.test(trimmed)) {
...
export async function runFullVerifyRace(context) {
  ...
  // STRICT: context.date is already canonical ISO format from handler
  // If it's already YYYY-MM-DD, use it EXACTLY as-is (no modification)
  // Only normalize if it's missing or not in ISO format
  const requestDateIso = (date || "").trim();
  let targetDateIso = requestDateIso;
  
  // Safety check: if already ISO format, use it exactly (no re-normalization)
  if (targetDateIso && /^\d{4}-\d{2}-\d{2}$/.test(targetDateIso)) {
    // Already canonical - use as-is (STRICT: no modification)
    targetDateIso = targetDateIso;
    console.log("[runFullVerifyRace] Using canonical date as-is:", targetDateIso);
  } else if (!targetDateIso) {
    // Missing - fall back to today (only when truly missing)
    targetDateIso = getCanonicalRaceDate(null);
    console.warn("[runFullVerifyRace] Date missing, falling back to today:", targetDateIso);
  } else {
    // Not ISO format - normalize it (shouldn't happen if handler is correct)
    console.warn("[runFullVerifyRace] Date not in ISO format, normalizing:", requestDateIso);
    targetDateIso = getCanonicalRaceDate(requestDateIso);
  }
