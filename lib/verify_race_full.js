...
function parseHRNRaceOutcome($, raceNo, track = "") {
  ...
  try {
    ...
    // Only return if all three are valid
    if (!isValidOutcome(outcome) || !outcome.win || !outcome.place || !outcome.show) {
      return {};
    }
  } catch (error) {
...
async function extractOutcomeFromResultPage(url, ctx) {
  try {
    ...
    const $ = cheerio.load(cleaned);
    const hrnOutcome = parseHRNRaceOutcome($, ctx.raceNo, ctx.track);
    
    // Only return if outcome is valid and has all three positions
    if (isValidOutcome(hrnOutcome) && hrnOutcome.win && hrnOutcome.place && hrnOutcome.show) {
      return hrnOutcome;
    }
  ...
  try {
    ...
    try {
      ...
      if (equibaseResult && equibaseResult.outcome && isValidOutcome(equibaseResult.outcome) &&
          equibaseResult.outcome.win && equibaseResult.outcome.place && equibaseResult.outcome.show) {
    ...
    // HRN-SECOND: Try HRN if Equibase failed
    // Build HRN URL directly (more reliable than CSE search)
    let hrnOutcome = null;
    let hrnUrl = null;
    
    try {
      // Build HRN entries-results URL pattern
      // Format: https://entries.horseracingnation.com/entries-results/{track-slug}/{date}
      const trackSlug = safeTrack.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "");
      const hrnBaseUrl = `https://entries.horseracingnation.com/entries-results/${trackSlug}/${safeDate}`;
      
      // Try fetching HRN page directly
      const hrnRes = await fetch(hrnBaseUrl, {
        headers: {
          "User-Agent": "Mozilla/5.0 (FinishLineVerifyBot)",
        },
      });
      ...
      if (hrnRes && hrnRes.ok) {
        ...
        const cleaned = cleanHtml(hrnHtml);
        const $ = cheerio.load(cleaned);
        
        const parsed = parseHRNRaceOutcome($, safeRaceNo, safeTrack);
        
        if (parsed && isValidOutcome(parsed) && parsed.win && parsed.place && parsed.show) {
          hrnOutcome = parsed;
          hrnUrl = hrnBaseUrl;
        }
      }
    } catch (hrnError) {
      console.error("[verify_race] HRN direct fetch failed, falling back to CSE", {
    ...
    // If HRN succeeded, return immediately
    if (hrnOutcome && hrnUrl) {
      const outcome = hrnOutcome;
      ...
      summaryLines.push(`Using date: ${safeDate}`);
      summaryLines.push(`Step: verify_race`);
      summaryLines.push(`Query: HRN entries-results: ${safeTrack} ${safeDate} Race ${safeRaceNo}`);
      summaryLines.push(`Top Result: HRN results -> ${hrnUrl}`);
      ...
      const predictedParts = [predictedSafe.win, predictedSafe.place, predictedSafe.show].filter(Boolean);
      if (predictedParts.length) {
        summaryLines.push(`Predicted: ${predictedParts.join(" / ")}`);
      } else {
        summaryLines.push("Predicted: (none)");
      ...
      return {
        ...
        track: safeTrack,
        raceNo: safeRaceNo,
        query: `HRN entries-results: ${safeTrack} ${safeDate} Race ${safeRaceNo}`,
        top: {
          title: "HRN results",
          link: hrnUrl,
        },
        ...
        debug: {
          source: "hrn",
          hrnUrl,
          googleUrl: buildGoogleSearchUrl(`HRN ${safeTrack} ${safeDate} Race ${safeRaceNo}`),
        },
      };
    }

    // Both Equibase and HRN failed - return null to trigger stub fallback
    return {
      ...defaultResponse,
      step: "verify_race_full_fallback",
      summary: `Full parser attempted but failed: Equibase and HRN parsing both failed. Using stub fallback (Google search only).`,
      debug: {
        googleUrl: buildGoogleSearchUrl(`${safeTrack} ${safeDate} Race ${safeRaceNo}`),
      },
    };
