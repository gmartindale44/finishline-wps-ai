 ...
 * @returns {string} - Canonical date in YYYY-MM-DD format, or today's date if rawDate is missing/empty
 */
// Simple formatter: accepts either ISO (YYYY-MM-DD) or MM/DD/YYYY and returns ISO.
// DOES NOT change the calendar day, only the formatting.
function toIsoRaceDate(raw) {
  if (!raw) return null;
  ...
  // Fallback: don't invent a date, just return null so caller can error out.
  return null;
}

// This is the ONLY place where canonicalDateIso is derived.
export function getCanonicalRaceDate(context) {
  // Prefer the date coming from the UI / request body.
  const uiDate =
    context?.raceDate ??
    context?.date ??
    context?.canonicalDate ??
    null;

  const iso = toIsoRaceDate(uiDate);
  if (!iso) {
    throw new Error("getCanonicalRaceDate: missing or invalid race date");
  }
  return iso;
}
...
export function parseHRNRaceOutcome(html, ctx = {}) {
  ...
  const raceNo =
    Number(ctx.race ?? ctx.raceNo ?? ctx.raceNumber ?? 0) || 0;
  ...
  const trackRaw =
    ...
    ctx.trackLabel ??
    "";
  const trackNorm = trackRaw.toString().toLowerCase().trim();

  // Detect "WPS" style tables: header must contain Win / Place / Show.
  function isWpsTable(table) {
    const headerRow = $(table).find("tr").first();
    const headerText = norm(headerRow.text());
    return (
  ...
  // Score candidate tables by how well their nearest heading matches
  // the requested race number and track name.
  const candidates = [];
  ...
  $("table").each((_, table) => {
    ...
    const $table = $(table);
    let score = 0;
    let sawRaceMatch = false;
    let sawTrackMatch = false;

    // Walk backwards from the table and look for heading-ish elements.
    let $probe = $table.prev();
    for (let hops = 0; hops < 40 && $probe.length; hops++) {
      if ($probe.is("table")) break; // don't cross other tables

      const text = norm($probe.text());
      if (!text) {
        $probe = $probe.prev();
        continue;
      }

      if (!sawRaceMatch && raceNo > 0) {
        const num = String(raceNo);
        const patterns = [
          `race ${num} `,
          `race #${num}`,
          `race ${num},`,
      ...
      if (!sawTrackMatch && trackNorm) {
        if (text.includes(trackNorm)) {
          sawTrackMatch = true;
          score += 5;
        }
      }
    ...
    // If we didn't see any heading context but we have no raceNo,
    // give a minimal score so it is still considered.
    if (score === 0 && raceNo === 0 && !trackNorm) {
      score = 1;
    }

    candidates.push({ table: $table, score });
  });
  ...
  // Use the best-scoring table (closest matching Race # and track).
  candidates.sort((a, b) => b.score - a.score);
  const wpsTable = candidates[0].table;

  // Extract first 3 runners as Win / Place / Show.
  const rows = wpsTable.find("tr").slice(1); // skip header row
  const horses = [];
  ...
  rows.each((_, tr) => {
    ...
    // HRN has horse name in first cell (with speed figure in parentheses).
    let name = $(cells[0]).text() || "";
    // Strip speed figure in parentheses, e.g. "Laugh Like Lucy (96*)"
    name = name.replace(/\([^)]*\)/g, "").trim();

    if (!name) return;

    if (typeof normalizeHorseName === "function") {
...
export async function runFullVerifyRace(context) {
  ...
  // Use canonical date from context - no modifications, no fallbacks to "today"
  const canonicalDateIso = context?.date || context?.raceDate || null;
  
  if (!canonicalDateIso) {
    return {
      ok: false,
      step: "verify_race_full_error",
      error: "Missing race date",
      summary: "Race date is required but was not provided.",
      outcome: { win: "", place: "", show: "" },
    };
  }
  
  // This MUST be the ONLY date used everywhere
  const finalDate = canonicalDateIso;
  ...
  try {
    ...
    const searchResult = await searchRaceLinks(req, safeTrack, finalDate, safeRaceNo);
    ...
    // Use the canonical date that was already extracted at the start
    const fallbackDate = finalDate;
    ...
    // The cseQuery should already use finalDate, but double-check it contains the correct date
    let finalQuery = cseQuery;
    if (finalDate && !cseQuery.includes(finalDate)) {
      // Rebuild query with canonical date to ensure consistency
      const dateVariants = formatDateVariants(finalDate);
