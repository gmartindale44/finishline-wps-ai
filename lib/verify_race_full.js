...
function findRaceWpsTable($, ctx) {
  const rn = Number(ctx?.race ?? ctx?.raceNo ?? ctx?.raceNumber);
  if (!rn) return null;

  const track = (ctx?.track ?? "").toLowerCase();
  const racePattern = new RegExp(`race\\s*#?\\s*${rn}\\b`, "i");

  let heading = null;
  $("h1, h2, h3, h4, h5, h6, strong, b, p, div").each((i, el) => {
    const txt = $(el).text().toLowerCase();
    if (racePattern.test(txt)) {
      if (track && !txt.includes(track.toLowerCase())) return;
      heading = el;
      return false;
    }
  });
  ...
  $(heading)
    ...
    .each((i, t) => {
      ...
      if (header.includes("win") && header.includes("place") && header.includes("show")) {
        table = t;
        return false;
      }
    });
...
export function parseHRNRaceOutcome(html, ctx) {
  ...
  const table = findRaceWpsTable($, ctx);
  if (!table) {
    return { win:"", place:"", show:"" };
  }
  ...
  rows.each((i, r) => {
    const raw = $(r).find("td").first().text().trim();
    if (raw) {
