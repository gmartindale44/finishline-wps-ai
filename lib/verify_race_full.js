...
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  // Extract race number from context (supports both raceNo and race properties)
  const raceNo = (ctx.raceNo || ctx.race || null) != null ? String(ctx.raceNo || ctx.race).trim() : null;
  ...
  // Helper: check if header text contains all required WPS columns
  function headerHasWps(text) {
    const t = lower(text);
    return t.includes("runner") && t.includes("win") && t.includes("place") && t.includes("show");
  }

  // ---------- 1) Locate the correct WPS payout table for *this* race ----------
  function findRaceScopedWpsTable() {
    if (!raceNo) return null;

    const raceRegex = new RegExp(`\\bRace\\s*${raceNo}\\b`, "i");

    // Find the heading/paragraph that mentions "Race <n>" in various tags
    const raceHeading = $("h1,h2,h3,h4,h5,h6,strong,b,p,div")
      .filter((_, el) => raceRegex.test($(el).text()))
      .first();

    if (!raceHeading.length) {
      return null;
    }

    // From that heading, walk forward through tables until we find a WPS table.
    let chosen = null;
    raceHeading.nextAll("table").each((_, table) => {
      if (chosen) return;
      ...
      // Must contain all required headers (runner, win, place, show)
      // Do NOT exclude "Runner (Speed)" tables - that IS the table we want
      if (!headerHasWps(headerText)) return;
  ...
  // Fallback: global search if we can't find a race-scoped table.
  ...
  let wpsTable = findRaceScopedWpsTable();
  if (!wpsTable) {
    wpsTable = findAnyWpsTable();
  }
