...
// Detect whether a table looks like an HRN Win/Place/Show table
function isWpsTable($, table) {
  ...
  // Use the first row as header (HRN uses <th>, but sometimes <td>)
  ...
  const hasRunner =
    headers.some((h) => h.includes("runner")) ||
    headers.some((h) => h.includes("horse"));
  ...
  // We want the classic WPS layout; runner column + W/P/S columns.
  return hasRunner && hasWin && hasPlace && hasShow;
}

// Find the WPS table that belongs to a specific race number (Race 1, Race 2, etc.)
function findRaceScopedWpsTable($, raceNumber) {
  if (!raceNumber && raceNumber !== 0) return null;

  const normalizedRace = String(raceNumber).trim();
  if (!normalizedRace) return null;

  // Build a few text patterns HRN typically uses
  const patterns = [
    `race ${normalizedRace}`,
    `race #${normalizedRace}`,
    `race ${normalizedRace}:`,
  ...
  // Any element that might contain "Race 1" text: headings, bold, paragraphs, etc.
  const headingSelector = "h1,h2,h3,h4,h5,h6,strong,b,p,div";

  const headings = $(headingSelector).filter((_, el) => {
    const text = $(el)
      .text()
      .trim()
      .toLowerCase()
  ...
  // Use the *first* matching heading; walk forward to the first WPS table.
  for (let i = 0; i < headings.length; i++) {
    const el = headings[i];
    let $cursor = $(el).next();

    while ($cursor && $cursor.length) {
      // Skip non-table elements until we hit a table or another heading/race section
      if ($cursor.is("table")) {
        if (isWpsTable($, $cursor)) {
          return $cursor;
        }
      }

      // If we hit another obvious race header block, stop scanning for this heading
      if ($cursor.is(headingSelector)) {
        break;
      }
...
// Fallback: any WPS table on the page (used only if race-scoped search fails)
function findAnyWpsTable($) {
...
// HRN WPS parsing: clean, deterministic algorithm for extracting Win/Place/Show from HRN results pages.
// The HRN table is labeled "Runner (Speed)" with headers: Runner (Speed) | Win | Place | Show
// For finished races: Row 1 = Win, Row 2 = Place, Row 3 = Show (all have numeric payouts)
function parseHRNRaceOutcome($, ctx = {}) {
  ...
  // Determine the race number from context. Use whatever existing fields are present.
  const raceNumber =
    (ctx && (ctx.race || ctx.raceNumber || ctx.raceNo)) !== undefined
      ? ctx.race || ctx.raceNumber || ctx.raceNo
      : null;

  const wpsTable =
    findRaceScopedWpsTable($, raceNumber) ?? findAnyWpsTable($);

  if (!wpsTable) {
    // If we cannot find a WPS table at all, signal "no outcome" so caller can fall back.
    return { win: "", place: "", show: "" };
  }

  // From here down, keep existing logic:
  // - figure out idxRunner / idxWin / idxPlace / idxShow from wpsTable headers
  // - iterate rows to produce winHorse / placeHorse / showHorse
  // - apply any existing fallbacks

  const $table = $(wpsTable);

  // ---------- 2) Map column indices for Runner/Win/Place/Show ----------
  ...
  headerCells.each((i, cell) => {
    const text = lower($(cell).text());

    if (text.includes("runner")) runnerCol = i;
    if (text.includes("win")) winCol = i;
    if (text.includes("place")) placeCol = i;
    if (text.includes("show")) showCol = i;
  });

  // Fallback if headers are a little weird: assume classic layout (only if we have 4+ columns)
  if ((runnerCol === -1 || winCol === -1 || placeCol === -1 || showCol === -1) && headerCells.length >= 4) {
    runnerCol = 0;
    winCol = 1;
    placeCol = 2;
    showCol = 3;
  ...
  // If we still don't have a coherent column mapping, return early
  if (runnerCol === -1 || winCol === -1 || placeCol === -1 || showCol === -1) {
    return { win: null, place: null, show: null };
  }

  // ---------- 3) Row scanning and payout detection ----------
  function hasNumericPayout(text) {
    if (!text) return false;
  ...
  rows.each((i, row) => {
    ...
    const runnerCell = cells.eq(runnerCol);
    if (!runnerCell.length) return;

    const rawName = runnerCell.text();
    const horseName = normalizeHorseName(rawName);
    if (!horseName) return;

    orderedHorses.push(horseName);

    const winTxt = winCol >= 0 ? cells.eq(winCol).text() : "";
    const placeTxt = placeCol >= 0 ? cells.eq(placeCol).text() : "";
    const showTxt = showCol >= 0 ? cells.eq(showCol).text() : "";

    if (!winHorse && hasNumericPayout(winTxt)) {
      winHorse = horseName;
    }

    if (!placeHorse && horseName !== winHorse && hasNumericPayout(placeTxt)) {
      placeHorse = horseName;
    }

    if (!showHorse && horseName !== winHorse && horseName !== placeHorse && hasNumericPayout(showTxt)) {
      showHorse = horseName;
    }
  });

  // ---------- 4) Simple reading-order fallback for Show ----------
  // If no showHorse yet but we have at least win/place, use reading order.
  if (!showHorse && orderedHorses.length) {
    const primary = placeHorse || winHorse;
    if (primary) {
      const idx = orderedHorses.findIndex((h) => h === primary);
      if (idx >= 0) {
        for (let i = idx + 1; i < orderedHorses.length; i++) {
