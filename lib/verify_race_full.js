...
// Normalize generic text (not horse names) for matching headers / headings.
function normalizeText(txt) {
  return (txt || "").toString().replace(/\s+/g, " ").trim().toLowerCase();
}

// Detect whether a table is a Win / Place / Show table (Runner (Speed) block).
function isWpsTable($, table) {
  const headerRow = $(table).find("tr").first();
  if (!headerRow.length) return false;
  const headerText = normalizeText(headerRow.text());

  // We only care that the header includes win/place/show somewhere.
  // We *do not* enforce "Runner (Speed)" text, but it's usually present.
  return (
...
/**
 * Find the WPS table for the requested race.
 *
 * Strategy:
 *   1. Use ctx.race / ctx.raceNumber / ctx.raceNo to build a "Race 1" style pattern.
 *   2. Scan heading-like elements (h1–h6, strong, b, p, div) for a match that also
 *      mentions the track name, if provided (e.g. "Parx Racing Race # 1").
 *   3. Starting from that heading, walk forward through sibling elements and
 *      choose the first table that looks like a WPS table (has win/place/show header).
 *   4. If that fails, fall back to "any WPS table on the page" as a last resort.
 */
function findRaceScopedWpsTable($, ctx = {}) {
  const raceNumber =
    ctx?.race ?? ctx?.raceNumber ?? ctx?.raceNo ?? null;
  const rawTrack =
    ctx?.track ?? ctx?.trackName ?? ctx?.track_label ?? ctx?.trackLabel ?? "";

  const trackNorm = normalizeText(rawTrack);

  const racePatterns = [];
  if (raceNumber != null) {
    // Race 1, Race #1, Race 1,
    racePatterns.push(new RegExp(`race\\s*#?\\s*${raceNumber}\\b`, "i"));
  }
  ...
  $(headingSelector).each((_, el) => {
    if (wpsTable) return; // already found

    const text = normalizeText($(el).text());
    if (!text) return;

    // Must match the race pattern if we have one.
    if (racePatterns.length) {
      const matchesRace = racePatterns.some((re) => re.test(text));
      if (!matchesRace) return;
    } else if (!text.includes("race")) {
      // No explicit race number in ctx; at least look for the word "race".
      return;
    }

    // If we know the track, ensure the same heading text also mentions it.
    if (trackNorm && !text.includes(trackNorm)) {
      return;
    }

    // From this heading, walk forward and find the first WPS table.
    const tablesAhead = $(el).nextAll("table");
    tablesAhead.each((_, tbl) => {
      if (wpsTable) return false;
      if (isWpsTable($, tbl)) {
        wpsTable = $(tbl);
        return false;
      }
  ...
  // Last-resort fallback: any WPS table on the page.
  if (!wpsTable) {
    $("table").each((_, tbl) => {
      if (wpsTable) return false;
      if (isWpsTable($, tbl)) {
  ...
  return wpsTable || null;
}

function parseHRNRaceOutcome(html, ctx = {}) {
  const $ = cheerio.load(html);

  const wpsTable = findRaceScopedWpsTable($, ctx);
  if (!wpsTable || wpsTable.length === 0) {
    // Signal "no structured outcome" to the caller; fallback will handle it.
    return { win: "", place: "", show: "" };
  }

  // HRN Runner (Speed) tables always list the Win / Place / Show horses as the
  // first three rows of this table for a completed race.
  const rows = wpsTable.find("tr").slice(1); // skip header
  const horses = [];

  rows.each((_, row) => {
    if (horses.length >= 3) return; // we only need top 3
    const cells = $(row).find("td");
    if (!cells.length) return;
    const rawName = $(cells[0]).text();
    const name = normalizeHorseName(rawName);
    if (name) horses.push(name);
  });

  const [win, place, show] = horses;
  return {
    win: win || "",
    place: place || "",
    show: show || "",
...
function parseOutcomeFromHtml(html, url, raceNo = null) {
  ...
  try {
    ...
    if (isHRN && raceNo) {
      ...
      const hrnOutcome = parseHRNRaceOutcome(cleaned, { raceNo: String(raceNo), track: trackFromUrl });
