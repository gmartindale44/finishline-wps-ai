...
export async function runFullVerifyRace(context) {
  ...
  // PATCH: Remove ALL re-normalization
  // context.date is already locked in by handler - use it EXACTLY as-is
  // DO NOT MODIFY, DO NOT NORMALIZE, DO NOT TRANSFORM
  const requestDateIso = (date || "").trim();
  let targetDateIso = requestDateIso;
  
  // Diagnostic log
  console.log("[VERIFY_DATES] Parser input:", targetDateIso, "Context date:", date);
  
  // If date is empty, only then fall back to today
  if (!targetDateIso) {
    targetDateIso = getCanonicalRaceDate(null);
    console.warn("[runFullVerifyRace] Date missing, falling back to today:", targetDateIso);
  } else {
    // Date is provided - use it AS-IS (already locked in by handler)
    // DO NOT re-normalize - handler already locked it in
    console.log("[runFullVerifyRace] Using date AS-IS (no modification):", targetDateIso);
  }
