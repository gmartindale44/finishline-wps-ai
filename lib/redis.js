...
export async function sadd(key, members) {
  ...
  const sanitized = members.filter((m) => m != null && String(m).length > 0);
  if (!sanitized.length) return true;

  const pathSegment = sanitized.map((m) => encodeURIComponent(String(m))).join('/');
  try {
    const r = await fetch(`${url}/SADD/${encodeURIComponent(key)}/${pathSegment}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!r.ok) {
      const detail = await r.text().catch(() => `HTTP ${r.status}`);
...
export async function smembers(key) {
  ...
  try {
    ...
    return result.map((v) => (v == null ? '' : String(v)));
  } catch (e) {
    if (e.message && e.message.includes('redis_unreachable')) throw e;
    throw new Error(`redis_unreachable: ${String(e?.message || e)}`);
  }
