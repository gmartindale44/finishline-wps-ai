...
export async function hset(key, map) {
  ...
  // Convert map to array of [field, value] pairs, ensuring all values are strings
  const fieldValuePairs = Object.entries(map).flatMap(([field, value]) => {
    // Ensure value is always a string
    const stringValue = typeof value === 'string' ? value : JSON.stringify(value);
    return [String(field), stringValue];
  });
  
  // Use pipeline format for HSET with multiple fields (more reliable than URL path)
  // HSET key field1 value1 field2 value2 ...
  const command = ['HSET', key, ...fieldValuePairs];
  ...
  try {
    ...
    // Pipeline returns array of results, check first result for errors
    if (Array.isArray(result) && result[0] && result[0].error) {
      throw new Error(`redis_unreachable: ${result[0].error}`);
    }
