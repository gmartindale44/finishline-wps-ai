...
import { keys, hgetall, get } from "../redis.js";
 ...
 * Verify logs are stored as JSON strings (via redis.set), not hashes
 */
async function loadVerifyLog(key) {
  try {
    // Try reading as JSON string first (current format)
    const rawValue = await get(key);
    let verifyData = null;
    
    if (rawValue) {
      try {
        verifyData = typeof rawValue === "string" ? JSON.parse(rawValue) : rawValue;
      } catch {
        // If parsing fails, try reading as hash (legacy format)
        const hash = await hgetall(key);
      ...
      // Fallback to hash format if get returns null
      const hash = await hgetall(key);
      if (hash && Object.keys(hash).length > 0) {
        verifyData = hash;
      }
    ...
    if (!verifyData || typeof verifyData !== "object") return null;
    
    const raceKey = extractRaceKey(key);
    if (!raceKey) return null;
    
    // Extract outcome (handle both object and string formats)
    let outcome = {};
    if (verifyData.outcome) {
      if (typeof verifyData.outcome === "string") {
        try {
          outcome = JSON.parse(verifyData.outcome);
        } catch {
          outcome = {};
        }
      } else if (typeof verifyData.outcome === "object") {
        outcome = verifyData.outcome;
      }
    }
    
    // Fallback to individual fields if outcome object is empty
    if (!outcome.win && !outcome.place && !outcome.show) {
      outcome = {
        win: verifyData.outWin || verifyData.win || "",
        place: verifyData.outPlace || verifyData.place || "",
        show: verifyData.outShow || verifyData.show || "",
      };
    }
    
    // Parse hits - handle both boolean and string formats
    const hits = verifyData.hits || {};
    const winHit = hits.winHit === true || hits.winHit === "true" || verifyData.winHit === true || verifyData.winHit === "true" || verifyData.winHit === "1";
    const top3Hit = hits.top3Hit === true || hits.top3Hit === "true" || verifyData.top3Hit === true || verifyData.top3Hit === "true" || verifyData.top3Hit === "1";
    
    // Good outcome: win OR top3
    const goodOutcome = winHit || top3Hit;
    
    // Extract date - try multiple fields and fallback to raceKey
    const date = verifyData.date || verifyData.dateIso || (verifyData.debug && verifyData.debug.canonicalDateIso) || raceKey.date;
    
    return {
      key,
      ...raceKey,
      track: verifyData.track || raceKey.track,
      date: date || raceKey.date,
      raceNo: verifyData.raceNo || raceKey.raceNo,
      outcome,
      winHit,
      top3Hit,
      goodOutcome,
