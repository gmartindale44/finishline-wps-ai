...
export function parseEquibaseOutcome(html) {
  ...
  try {
    ...
    outcome.win = winRow.horse.trim();
    outcome.place = placeRow.horse.trim();
    outcome.show = showRow.horse.trim();

    // Only return non-null if all three positions are non-empty
