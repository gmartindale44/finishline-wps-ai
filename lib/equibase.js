...
export function parseEquibaseOutcome(html) {
  ...
  try {
    // Clean HTML first, then load into cheerio
    ...
    let finIdx = headerTexts.findIndex(
      (h) => h.includes("fin") || h === "fin" || h === "#" || h === "pos" || h === "position"
    );
    // Find horse name column - prefer "horse" or "horse name", avoid "jockey", "trainer", "owner"
    let horseIdx = headerTexts.findIndex(
      (h) =>
        (h.includes("horse") && !h.includes("jockey") && !h.includes("trainer")) ||
        h.includes("horse name") ||
        (h.includes("program") && !h.includes("jockey")) ||
        h.includes("pgm")
    );
    
    // If not found, try to find a column that's NOT jockey/trainer/owner
    if (horseIdx === -1) {
      const skipColumns = ["jockey", "trainer", "owner", "fin", "pos", "position", "win", "place", "show", "odds"];
      for (let i = 0; i < headerTexts.length; i++) {
        const h = headerTexts[i];
        if (!skipColumns.some(skip => h.includes(skip))) {
          // This might be the horse name column
          horseIdx = i;
          break;
        }
      }
    ...
    resultTable.find("tr").each((_, row) => {
      ...
      if (horseIdx >= 0 && horseIdx < cells.length) {
        // Get text from the cell, prefer link text if available (horse names are often in links)
        const $cell = $(cells[horseIdx]);
        horseText = $cell.find("a").first().text().trim() || $cell.text().trim();
      } else if (cells.length > 1) {
        // Try second column, but avoid jockey columns
        const $cell = $(cells[1]);
        const cellText = $cell.text().trim();
        // Skip if it looks like a jockey name pattern
        if (!/^[A-Z]\.\s+[A-Za-z'-]+$/.test(cellText)) {
          horseText = $cell.find("a").first().text().trim() || cellText;
        }
      }
      ...
      // Clean horse name: remove program numbers, extra spaces, jockey names, payouts, speed figs
      ...
      // Remove any text starting with $ or pure numbers/decimals
      horse = horse.replace(/^\$?\d+(\.\d+)?\s*/, "").trim();
      // Remove trailing asterisks (speed fig markers)
      horse = horse.replace(/\*+$/, "").trim();
      // Remove any text containing "picks", "stats", "winning", "percent", "payout"
    ...
    // Find Fin 1, 2, 3 - allow partial outcomes
...
/**
 * Parse Equibase outcome from a URL
 * @param {string} url - Equibase chart URL
 * @param {{ track?: string; date?: string; raceNo?: string | number }} context - Optional context for logging
 * @returns {Promise<{ win?: string; place?: string; show?: string; source: "equibase"; link: string; title?: string } | null>}
 */
export async function parseEquibaseOutcomeFromUrl(url, context = {}) {
  try {
    if (!url || typeof url !== "string") {
      return null;
    }
    ...
    const hasWin = outcome.win && outcome.win.trim();
    const hasPlace = outcome.place && outcome.place.trim();
    const hasShow = outcome.show && outcome.show.trim();

    if (!hasWin && !hasPlace && !hasShow) {
      return null;
    }
    ...
    console.error("[equibase] parseEquibaseOutcomeFromUrl error", {
      url,
      context,
      error: err?.message || String(err),
    });
...
export async function resolveEquibaseOutcome(context) {
  try {
    ...
    const result = await parseEquibaseOutcomeFromUrl(url, context);
    if (!result) {
      return null;
    }
