...
function computeBrierScore(rows) {
  ...
  for (const row of rows) {
    // Use raw_confidence_pct for raw Brier score
    const rawConfidence = row.raw_confidence_pct;
...
export function computeVerifyMetricsV1(rows, options = {}) {
  ...
  // Use raw_confidence_pct if available, otherwise fall back to confidence_pct as raw
  const rowsForRecalibration = normalizedRows.map((row) => ({
    ...row,
    raw_confidence_pct: row.raw_confidence_pct ?? row.confidence_pct, // Fallback to confidence_pct if raw_confidence_pct missing
  }));
  ...
  const rowsWithCalibrated = normalizedRows.map((row) => {
    const rawConf = row.raw_confidence_pct ?? row.confidence_pct;
