 ...
 * Extracts program numbers (horse numbers) based on payout presence
 * @param {string} html
 * @param {string|number|null} [raceNo] - Optional race number to scope to
 * @returns {{ outcome: ParsedOutcome, debugNotes: Array }}
 */
export function parseHrnWpsOutcome(html, raceNo = null) {
  const outcome = { win: "", place: "", show: "" };
  const debugNotes = [];

  try {
    // If raceNo is provided, try to scope HTML to that race
    let scopeHtml = html;
    if (raceNo != null && raceNo !== "") {
      const num = String(raceNo).trim();
      try {
        // Try to find the race section
        const racePattern = new RegExp(
          `Race\\s*#?\\s*${num}[\\s\\S]{0,10000}?(?=Race\\s*#?\\s*\\d+|$)`,
          "i"
        );
        const match = html.match(racePattern);
        if (match) {
          scopeHtml = match[0];
        }
      } catch {
    ...
    for (const table of tables) {
      ...
      // Check for required headers - be flexible with header text matching
      const runnerIdx = headers.findIndex((h) => h.includes("runner") || h.includes("horse"));
      // Match "win", "place", "show" more flexibly (case-insensitive, allow whitespace variations)
      const winIdx = headers.findIndex((h) => h.trim() === "win" || h === "win");
      const placeIdx = headers.findIndex((h) => h.trim() === "place" || h === "place");
      const showIdx = headers.findIndex((h) => h.trim() === "show" || h === "show");
    ...
    if (!targetTable) {
      // Try to find any table with runner and payout columns as fallback
      for (const table of tables) {
        const $table = $(table);
        const $headerRow = $table.find("tr").first();
        const $headerCells = $headerRow.find("th, td");
        ...
        if (hasRunner && (hasWin || hasPlace || hasShow)) {
          // Found a table with runner and at least one payout column
          // Try to extract indices more flexibly
          const runnerIdx = headers.findIndex((h) => h.includes("runner") || h.includes("horse"));
          ...
          if (runnerIdx !== -1) {
            targetTable = $table;
            idxRunner = runnerIdx;
            idxWin = winIdx >= 0 ? winIdx : -1;
            idxPlace = placeIdx >= 0 ? placeIdx : -1;
            idxShow = showIdx >= 0 ? showIdx : -1;
            break;
          }
        }
      }
    ...
    $rows.each((_, row) => {
      ...
      const winVal = idxWin >= 0 ? (cells[idxWin] || "") : "";
      const placeVal = idxPlace >= 0 ? (cells[idxPlace] || "") : "";
      const showVal = idxShow >= 0 ? (cells[idxShow] || "") : "";

      const hasWinPayout = idxWin >= 0 && isPayoutText(winVal);
      const hasPlacePayout = idxPlace >= 0 && isPayoutText(placeVal);
      const hasShowPayout = idxShow >= 0 && isPayoutText(showVal);
...
export async function fetchAndParseResults(url, options = {}) {
  ...
  try {
    ...
    if (isHRN) {
      try {
        // Try new robust parser first (pass raceNo for scoping)
        const parsed = parseHrnWpsOutcome(html, raceNo);
