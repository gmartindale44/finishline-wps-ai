...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { horses, meta, features } = req.body || {};
    if (!Array.isArray(horses) || horses.length === 0) {
      return res.status(400).json({ error: 'No horses provided' });
    }

    // If caller sent features from /analyze, use them; otherwise compute quickly here
    ...
    // Rank by score desc; break ties by impliedProb then name
    ...
    // confidence is a simple margin between #1 and #2
