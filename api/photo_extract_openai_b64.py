...
def _extract_horses_from_image(img_bytes: bytes) -> list:
    """Extract horse data from image using OpenAI Vision API."""
    try:
        # Get OpenAI API key
        ...
        prompt = (
            ...
            "name, odds, jockey, trainer. Return JSON only in this format: "
            '{"horses": [{"name": "Horse Name", "odds": "5-2", "jockey": "Jockey Name", "trainer": "Trainer Name"}]}'
        )
        ...
        completion = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are an OCR parser for race sheets. Extract horse data accurately."},
                {
                    "role": "user",
                    "content": [
...
async def photo_extract_handler(request: Request):
    try:
        ...
        # Extract horses using OpenAI Vision API
        horses = _extract_horses_from_image(img_bytes)
        
        # success — return horses data
        return JSONResponse({
            "ok": True,
